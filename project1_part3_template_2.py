...
def train_net(net, trainloader, logging, criterion, optimizer, scheduler, epochs=6):
    net = net.train()
    for epoch in range(epochs):  # loop over the dataset multiple times, only 1 time by default
        ...
        if type(scheduler).__name__ != 'NoneType':
            scheduler.step()
    # save network
    torch.save(modified.state_dict(), 'baseline.pth')
...
class Baseline(nn.Module):
    def __init__(self):
        ...
        self.pool = nn.MaxPool2d(2, 2)
        self.fc1 = nn.Linear(128 * 8 * 8, 32 * 2 * 2)
        self.fc2 = nn.Linear(32 * 2 * 2, 120)
...
scheduler = StepLR(optimizer, step_size=3, gamma=0.5) # define your own lr schedulers
